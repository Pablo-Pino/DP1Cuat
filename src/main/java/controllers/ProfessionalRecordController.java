...
@Controller
@RequestMapping("sponsor/professionalRecord")
public class ProfessionalRecordController extends AbstractController {
	 ...
	 * ModelAndView res;
	 * ProfessionalRecord profRec;
	 * profRec = this.professionalRecordService.create();
	 * 
	...
	public ModelAndView edit(@RequestParam final int sponsorId) {
		ModelAndView res;
		ProfessionalRecord sponsor;

		sponsor = this.professionalRecordService.findOne(sponsorId);
	...
	public ModelAndView save(@Valid final ProfessionalRecord sponsor, final BindingResult binding) {
		ModelAndView result;
		...
		if (binding.hasErrors())
			...
			try {
				final ProfessionalRecord aud = this.professionalRecordService.save(sponsor);
	...
	public ModelAndView list() {
		...
		profRecs = this.handyWorkerService.findAll(); //TODO probar con el servicio que da directamente los sponsors del professionalRecord

		res.addObject("sponsors", profRecs);
		res.addObject("requestURI", "sponsor/professionalRecord/list.do");
	...
	private ModelAndView createEditModelAndView(final ProfessionalRecord sponsor) {
	...
	protected ModelAndView createEditModelAndView(final ProfessionalRecord sponsor, final String message) {
		ModelAndView result;
	...
	//	private ModelAndView createEditModelAndView(final ProfessionalRecord sponsor, final String string) {
	...
	//
	//		if (sponsor.getProfessionalRecord() == null) {
	//
	//			momentOfCreate = null;
	//			title = null;
	//			description = null;
